...
matrix44<T> quat_to_matrix(quaternion<T> const& q)
{
    ...
    res[0][0] = 2 * (w2 + q.x() * q.x()) - 1;
    res[0][1] = 2 * (xy - wz);
    res[0][2] = 2 * (xz + wy);
    res[0][3] = 0;

    res[1][0] = 2 * (xy + wz);
    res[1][1] = 2 * (w2 + q.y() * q.y()) - 1;
    res[1][2] = 2 * (yz - wx);
    res[1][3] = 0;

    res[2][0] = 2 * (xz - wy);
    res[2][1] = 2 * (yz + wx);
    res[2][2] = 2 * (w2 + q.z() * q.z()) - 1;
    res[2][3] = 0;
...
quaternion<T> matrix_to_quat(matrix44<T> const& m)
{
    ...
    else
    {
        ...
        if (i == 0)
        {
            res.x(std::sqrt(1 + m[0][0] - m[1][1] - m[2][2]) * T(0.5));
        ...
        {
            res.y(std::sqrt(1 - m[0][0] + m[1][1] - m[2][2]) * T(0.5));
        ...
        {
            res.z(std::sqrt(1 - m[0][0] - m[1][1] + m[2][2]) * T(0.5));
